...
class CreditCardStrategy(PaymentStrategy):
    ...
    def pay(self, amount: float) -> dict:
        ...
        print(f"Processando ${amount:.2f} com Cartão de Crédito (FastAPI)")
        # Lógica real de integração com gateway de cartão de crédito iria aqui
        return {"status": "success", "method": "creditcard", "amount": amount, "framework": "FastAPI"}

class PayPalStrategy(PaymentStrategy):
    """Estratégia concreta para pagamento com PayPal."""
    def pay(self, amount: float) -> dict:
        """Simula o processamento de pagamento com PayPal."""
        print(f"Processando ${amount:.2f} com PayPal (FastAPI)")
        # Lógica real de integração com API do PayPal iria aqui
        return {"status": "success", "method": "paypal", "amount": amount, "framework": "FastAPI"}

# Contexto
class PaymentProcessor:
    """Contexto que utiliza uma estratégia de pagamento.
